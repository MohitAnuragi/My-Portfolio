...
const baseModelUrl = (fileName) => `${import.meta.env.BASE_URL}models/${fileName}`
...
const IPhoneModel = ({ activeAppId, setActiveAppId, ...props }) => {
    ...
    const { scene } = useGLTF(baseModelUrl('iphone.glb'))
...
const PixelModel = ({ activeAppId, setActiveAppId, ...props }) => {
    ...
    const { scene } = useGLTF(baseModelUrl('google_pixel_phone.glb'))
    const clone = useMemo(() => scene.clone(), [scene])
...
const Flip7Model = ({ activeAppId, setActiveAppId, ...props }) => {
    ...
    return (
        <group {...props} dispose={null} onClick={() => setIsFolded(!isFolded)}>
            ...
            {/* Top Half */}
            <AnimatedGroup position-z={zOffset} rotation-x={rotationX}>
...
// Preload models
useGLTF.preload(baseModelUrl('iphone.glb'))
useGLTF.preload(baseModelUrl('google_pixel_phone.glb'))
